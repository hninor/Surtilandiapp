...
public class Query<T> implements Iterable<T> {
    ...
    public Query<T> addParameter(String name, String value) {
        Query<T> result = new Query<>(this.getService(), this.getSubpath(),
        ...
        if (!isExecuted()) {
            ...
            if (resource.getStatus().isSuccess()) {
                ...
                case TYPE_ENTITY_SET:
                    FeedContentHandler<T> feedContentHandler = new FeedContentHandler<>(
                    ...
                    break;
                case TYPE_ENTITY:
                    EntryContentHandler<T> entryContentHandler = new EntryContentHandler<>(
                            entityClass, entityType, metadata, getLogger());
                    Feed feed = new Feed();
                    feed.getEntries().add(
                ...
                case TYPE_UNKNOWN:
                    ...
                    if (string.contains("<feed")) {
                        feedContentHandler = new FeedContentHandler<>(
                                entityClass, entityType, metadata, getLogger());
    ...
    public Iterator<T> iterator() {
        ...
        try {
            ...
            if (getNextPage() != null) {
                result = new EntryIterator<>(this.service, result,

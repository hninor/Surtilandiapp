...
public class Generator {
	...
	public static void generate(String user, String password, String outputDirName,
			String dataServiceUri, String serviceName) {
		System.out.println("---------------------------");
		...
		if (errorMessage == null) {
			...
			if (dataServiceUri.endsWith("$metadata")) {
				dataServiceUri = dataServiceUri.substring(0,
						dataServiceUri.length() - 10);
			} else if (dataServiceUri.endsWith("/")) {
				dataServiceUri = dataServiceUri.substring(0,
						dataServiceUri.length() - 1);
			}

			Service service = new Service(dataServiceUri, user, password);
			...
			if (service.getMetadata() == null) {
				...
				try {
					svcUtil.generate(outputDir, user, password);
					System.out
     ...
     * @throws Exception
     */
    public void generate(File outputDir, String user, String password) throws Exception {
        Service service = new Service(serviceRef, user, password);
        ...
        //CustomDaoGenerator.buildGeneratorDao(metadata);
     ...
     * @throws Exception
     */
    public void generate(String outputDir, String user, String password) throws Exception {
        generate(new File(outputDir), user, password);
    }
}
